  ...
  add: function (declaration) {
    function addPart(acc, part) {
      if (part instanceof Array) {
        ...
        if (part.temp) {
          var temp = declaration[part.temp] || AvastSP.Templates[part.temp];
...
AvastSP.Templates.add({
   ...
   */
  topBar: [
    "<div class='avast-extension-topbar' style='background-color: {{color}} !important;'>",
      "<div class='avast-extension-topbar-icon' style='background-color: {{iconColor}} !important; background-image: url({{icon}}) !important;'></div>",
      ...
      "<div class='avast-extension-topbar-button'>",
        "<div class='avast-extension-topbar-button-text'>",
          "{{buttonText}}",
        "</div>",
      "</div>",
      "{{#declineText}}",
        "<div class='avast-extension-topbar-decline-button'>",
          "<div class='avast-extension-topbar-button-text'>",
            "{{declineText}} <em>{{domain}}</em>",
          "</div>",
        "</div>",
      "{{/declineText}}",
    "</div>"
...
{ // --- Templates Declaration
  safeShopBar : [[
    '<div class="avast-extension-safeshop-bar avast-sas {{#coupon}}avast-sas-coupon{{/coupon}} {{#discreet}}avast-sas-discreet{{/discreet}} {{#discreetSlider}}avast-sas-discreet-animated{{/discreetSlider}}">',
      ...
      '<h1 class="avast-sas-block avast-sas-logo avast-sas-orange">',
        '<a href="http://www.avast.com" class="avast-sas-icon" style="background-image: url({{images.logo}});">{{strings.sasProductName}}</a>',
      '</h1>',
      ...
      '{{#coupon}}',
        ...
        '{{#coupon.code}}',
          '<div class="avast-sas-block avast-sas-code avast-sas-orange">',
            '<h3><strong>{{strings.sasCouponCodeTitle}}</strong></h3>',
            '<div class="avast-sas-button avast-sas-code-button">{{coupon.code}}</div>',
            '<p>{{strings.sasClickToCopy}}</p>',
          '</div>',
        '{{/coupon.code}}',
  ...
  safeShopSettings : [
    ...
    '<ul class="avast-sas-right-drop" id="avast-sas-settings-drop">',
      ...
      '{{#settings}}',
        '<li class="avast-sas-settings-list avast-sas-text" id="{{actionId}}">{{label}}</li>',
      '{{/settings}}',
    '</ul>'
  ],
